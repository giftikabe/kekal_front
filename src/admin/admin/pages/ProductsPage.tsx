...
interface Product {
  id: string; collectionId: string; name: string; slug: string;
  description: string; mainImage: string; gallery: string[];
...
const EMPTY: Partial<Product> = {
  collectionId: "", name: "", slug: "", description: "",
  mainImage: "", gallery: [], colors: [], sizes: [], inStock: true, featured: false,
};
...
export default function ProductsPage() {
  ...
  const load = async () => {
    try {
      setLoading(true);
      const [p, c] = await Promise.all([productsApi.getAll(), collectionsApi.getAll()]);
      setProducts(p as Product[]); setCollections(c as Collection[]);
    } catch (e: any) { setError(e.message); } finally { setLoading(false); }
  };

  useEffect(() => { load(); }, []);

  const filtered = filterCollection ? products.filter((p) => p.collectionId === filterCollection) : products;
  ...
  const openEdit = async (p: Product) => {
    setSelected(p); setForm(p); setActiveTab("details");
    ...
    try {
      ...
      setSeoForm(existing || { route, metaTitle: p.name, metaDescription: p.description, keywords: [], socialImage: p.mainImage });
    } catch { setSeoForm({ route, metaTitle: p.name, metaDescription: p.description, keywords: [], socialImage: p.mainImage }); }
    setModal("edit");
  };
  ...
  const handleDelete = async () => {
    if (!selected) return;
    setSaving(true);
    try { await productsApi.delete(selected.id); await load(); closeModal(); }
    catch (e: any) { setError(e.message); } finally { setSaving(false); }
  };
    ...
    <div>
      <div className={ui.pageHeader}>
        ...
        <div style={{ display: "flex", gap: 10 }}>
          <select className={ui.select} style={{ width: 180 }} value={filterCollection} onChange={(e) => setFilterCollection(e.target.value)}>
            <option value="">All Collections</option>
            {collections.map((c) => <option key={c.id} value={c.id}>{c.name}</option>)}
          </select>
          {hasPermission("products", "create") && (
            <button className={`${ui.btn} ${ui.btnPrimary}`} onClick={openCreate}>+ New Product</button>
          )}
        </div>
      </div>
      ...
      {loading ? <div className={ui.loading}>Loading...</div> : filtered.length === 0 ? (
        <div className={ui.empty}><div className={ui.emptyIcon}>◻</div>No products yet.</div>
      ) : (
        <div className={ui.tableWrap}>
          <table className={ui.table}>
            <thead><tr><th>Image</th><th>Name</th><th>Collection</th><th>Colors</th><th>Sizes</th><th>Stock</th><th>Actions</th></tr></thead>
            <tbody>
              {filtered.map((p) => (
                <tr key={p.id}>
                  <td>{p.mainImage ? <img src={p.mainImage} alt={p.name} className={ui.imagePreview} /> : <div className={ui.noImage}>No img</div>}</td>
                  <td><strong>{p.name}</strong><div style={{ fontSize: 11, color: "#999" }}>{p.slug}</div></td>
                  <td style={{ fontSize: 12, color: "#666" }}>{getCollectionName(p.collectionId)}</td>
                  <td>
                    <div style={{ display: "flex", gap: 3, flexWrap: "wrap" }}>
                      {p.colors?.map((color) => (
                        <span key={color} style={{ width: 16, height: 16, background: color, border: "1px solid #eee", display: "inline-block" }} title={color} />
                      ))}
                    </div>
                  </td>
                  <td style={{ fontSize: 12, color: "#666" }}>{p.sizes?.join(", ")}</td>
                  <td><span className={`${ui.badge} ${p.inStock ? ui.badgeGreen : ui.badgeRed}`}>{p.inStock ? "Yes" : "No"}</span></td>
                  <td>
                    <div className={ui.actions}>
                      {hasPermission("products", "update") && <button className={ui.actionBtn} onClick={() => openEdit(p)}>Edit</button>}
                      {hasPermission("products", "delete") && <button className={`${ui.actionBtn} ${ui.actionBtnDanger}`} onClick={() => openDelete(p)}>Delete</button>}
                    </div>
                  </td>
                </tr>
              ))}
        ...
        <div className={ui.overlay}>
          <div className={ui.modal} style={{ maxWidth: 680 }}>
            <div className={ui.modalHeader}>
              <div className={ui.modalTitle}>{modal === "create" ? "New Product" : "Edit Product"}</div>
            ...
            <div className={ui.tabs} style={{ padding: "0 24px", borderBottom: "1px solid #eee" }}>
              ...
              <button className={`${ui.tab} ${activeTab === "media" ? ui.tabActive : ""}`} onClick={() => setActiveTab("media")}>Media & Variants</button>
            ...
            <div className={ui.modalBody}>
              {activeTab === "details" && (
                <div className={ui.form}>
                  <div className={ui.field}>
                    ...
                    <select className={ui.select} value={form.collectionId || ""} onChange={(e) => set("collectionId", e.target.value)}>
              ...
              {activeTab === "media" && (
                <div className={ui.form}>
                  <ImageUpload label="Main Image" value={form.mainImage || ""} onChange={(url) => set("mainImage", url)} />
                  <div className={ui.field}>
                    <label className={ui.label}>Gallery Images (comma-separated URLs)</label>
                    <textarea
                      className={ui.textarea}
                      style={{ minHeight: 70 }}
                      value={form.gallery?.join(", ") || ""}
                      onChange={(e) => set("gallery", e.target.value.split(",").map((v) => v.trim()).filter(Boolean))}
                      placeholder="https://url1.jpg, https://url2.jpg"
                    />
                    <div className={ui.hint}>Or upload individually via Cloudinary and paste URLs here</div>
                  </div>
                  <div className={ui.field}>
                  ...
                  <div className={ui.field}>
                    ...
                    <SizeSelector value={form.sizes || []} onChange={(sizes) => set("sizes", sizes)} />
                  </div>
                </div>
              )}
              {activeTab === "seo" && (
                <div className={ui.form}>
                  <div className={ui.field}>
                    ...
                    <input className={ui.input} value={seoForm.metaTitle} onChange={(e) => setSeoForm({ ...seoForm, metaTitle: e.target.value })} placeholder={form.name || "Product name"} />
                  </div>
                  <div className={ui.field}>
      ...
      {modal === "delete" && selected && (
        <div className={ui.overlay}>
          <div className={ui.modal}>
            <div className={ui.modalHeader}><div className={ui.modalTitle}>Delete Product</div><button className={ui.modalClose} onClick={closeModal}>✕</button></div>
            <div className={ui.modalBody}><div className={ui.confirmText}>Delete <strong>{selected.name}</strong>?</div><div className={ui.confirmSub}>This cannot be undone.</div></div>
            <div className={ui.modalFooter}>
              <button className={`${ui.btn} ${ui.btnSecondary}`} onClick={closeModal}>Cancel</button>
