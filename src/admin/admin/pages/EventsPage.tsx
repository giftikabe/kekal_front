...
import { eventsApi, pagesApi } from "../api/client";
...
interface Event {
  ...
  featuredImage: string; gallery: string[]; videoUrl?: string;
  category: string; type: "event" | "community-impact";
  eventDate: string; location: string; organizer: string; featured: boolean;
}
...
const EMPTY: Partial<Event> = {
  title: "", slug: "", intro: "", content: "", featuredImage: "", gallery: [],
  videoUrl: "", category: "exhibition", type: "event", eventDate: "", location: "", organizer: "", featured: false,
};
...
export default function EventsPage() {
  ...
  const load = async () => {
    try { setLoading(true); setEvents(await eventsApi.getAll() as Event[]); }
  ...
  const openDelete = (e: Event) => { setSelected(e); setModal("delete"); };
  ...
  const handleDelete = async () => {
    if (!selected) return;
    setSaving(true);
    try { await eventsApi.delete(selected.id); await load(); closeModal(); }
    catch (e: any) { setError(e.message); } finally { setSaving(false); }
  };

  const set = (key: keyof Event, value: any) => setForm((f) => ({ ...f, [key]: value }));

  return (
    <div>
      <div className={ui.pageHeader}>
        <div><div className={ui.pageTitle}>Events</div><div className={ui.pageCount}>{filtered.length} total</div></div>
        <div style={{ display: "flex", gap: 10 }}>
          <select className={ui.select} style={{ width: 160 }} value={filterType} onChange={(e) => setFilterType(e.target.value)}>
            <option value="">All Types</option>
            <option value="event">Event</option>
            <option value="community-impact">Community Impact</option>
          </select>
          {hasPermission("events", "create") && <button className={`${ui.btn} ${ui.btnPrimary}`} onClick={openCreate}>+ New Event</button>}
        </div>
      </div>
      ...
      {loading ? <div className={ui.loading}>Loading...</div> : filtered.length === 0 ? (
        <div className={ui.empty}><div className={ui.emptyIcon}>◷</div>No events yet.</div>
      ) : (
        <div className={ui.tableWrap}>
          <table className={ui.table}>
            <thead><tr><th>Image</th><th>Title</th><th>Category</th><th>Type</th><th>Date</th><th>Location</th><th>Actions</th></tr></thead>
            <tbody>
              {filtered.map((e) => (
                <tr key={e.id}>
                  ...
                  <td><span className={`${ui.badge} ${e.type === "community-impact" ? ui.badgeGreen : ui.badgeBlack}`}>{e.type}</span></td>
                  <td style={{ fontSize: 12 }}>{e.eventDate}</td>
                  <td style={{ fontSize: 12, color: "#666" }}>{e.location}</td>
                  <td>
                    <div className={ui.actions}>
                      {hasPermission("events", "update") && <button className={ui.actionBtn} onClick={() => openEdit(e)}>Edit</button>}
                      {hasPermission("events", "delete") && <button className={`${ui.actionBtn} ${ui.actionBtnDanger}`} onClick={() => openDelete(e)}>Delete</button>}
                    </div>
                  </td>
                </tr>
              ))}
        ...
        <div className={ui.overlay}>
          <div className={ui.modal} style={{ maxWidth: 640 }}>
            <div className={ui.modalHeader}>
              <div className={ui.modalTitle}>{modal === "create" ? "New Event" : "Edit Event"}</div>
            ...
            <div className={ui.modalBody}>
              {activeTab === "details" && (
                <div className={ui.form}>
                  ...
                  <div className={ui.field}><label className={ui.label}>Video URL</label><input className={ui.input} value={form.videoUrl || ""} onChange={(e) => set("videoUrl", e.target.value)} placeholder="https://youtube.com/embed/..." /></div>
                  <div className={ui.fieldRow}>
                    ...
                    <div className={ui.field}>
                      ...
                      <select className={ui.select} value={form.type || "event"} onChange={(e) => set("type", e.target.value)}>
                        <option value="event">Event</option>
                        <option value="community-impact">Community Impact</option>
                      </select>
                    </div>
      ...
      {modal === "delete" && selected && (
        <div className={ui.overlay}>
          <div className={ui.modal}>
            <div className={ui.modalHeader}><div className={ui.modalTitle}>Delete Event</div><button className={ui.modalClose} onClick={closeModal}>✕</button></div>
