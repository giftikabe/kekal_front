...
export default function CollectionsPage() {
  ...
  const canEdit = hasPermission("collections", "update");

  const load = async () => {
    try {
      setLoading(true);
      setCollections((await collectionsApi.getAll()) as Collection[]);
    } catch (e: any) {
      setError(e.message);
    } finally {
      setLoading(false);
  ...
  const handleDelete = async () => {
    ...
    setSaving(true);
    try {
      await collectionsApi.delete(selected.id);
      await load();
      closeModal();
    } catch (e: any) {
      setError(e.message);
  ...
  const set = (key: keyof Collection, value: any) =>
    ...
    <div>
      ...
      {loading ? (
        ...
        <div className={ui.empty}>
          <div className={ui.emptyIcon}>◫</div>No collections yet.
        </div>
      ) : (
            ...
            <tbody>
              {collections.map((c) => (
                ...
                >
                  ...
                  <td>
                    <span
                      className={`${ui.badge} ${c.status === "current" ? ui.badgeGreen : ui.badgeGray}`}
                    >
                      {c.status}
                    </span>
                  </td>
                  <td>
                    <span
                      className={`${ui.badge} ${c.featured ? ui.badgeBlack : ui.badgeGray}`}
                    >
                      {c.featured ? "Yes" : "No"}
                    </span>
                  </td>
                  <td>
                    <span
                      className={`${ui.badge} ${c.inStock ? ui.badgeGreen : ui.badgeRed}`}
                    >
                      {c.inStock ? "Yes" : "No"}
                    </span>
                  </td>
                  ...
                  <div className={ui.fieldRow}>
                    <div className={ui.field}>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          set("releaseYear", parseInt(e.target.value))
                        }
                      />
                    </div>
                    <div className={ui.field}>
                ...
                <div className={ui.form}>
                  <div className={ui.field}>
                    ...
                    <input
                      ...
                      placeholder={form.name || "Collection name"}
                    />
                  </div>
                  <div className={ui.field}>
                  ...
                  <ImageUpload
                    label="Social Image (defaults to cover image, then site logo, if left blank)"
      ...
      {modal === "delete" && selected && (
        <div className={ui.overlay}>
          <div className={ui.modal}>
            <div className={ui.modalHeader}>
              <div className={ui.modalTitle}>Delete Collection</div>
              <button className={ui.modalClose} onClick={closeModal}>
                ✕
              </button>
            </div>
            <div className={ui.modalBody}>
              ...
              <div className={ui.confirmSub}>
                This will also delete all products in this collection.
              </div>
            </div>
            <div className={ui.modalFooter}>
              <button
